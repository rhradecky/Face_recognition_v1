...
# Funkcia na spracovanie snímok
def process_frame(frame, yolo_model, known_face_encodings, known_face_names):
    # Zníženie rozlíšenia snímky na rýchlejšie spracovanie
    small_frame = cv2.resize(frame, (0, 0), fx=0.25, fy=0.25)

    # Detekcia objektov pomocou YOLOv5
    yolo_results = yolo_model(small_frame)
    ...
    return yolo_results, face_locations, face_names

# Funkcia na vykreslenie snímok
def draw_frame(frame, yolo_results, face_locations, face_names):
    # Kreslenie obrysov a výpis názvov na originálnu snímku
    for result in yolo_results:
        boxes = result.boxes
        for box in boxes:
            x_min, y_min, x_max, y_max = map(int, box.xyxy[0] * 4)  # Obnovenie rozlíšenia
            score = box.conf[0]
            label = int(box.cls[0])
    ...
    for (top, right, bottom, left), name in zip(face_locations, face_names):
        top, right, bottom, left = top * 4, right * 4, bottom * 4, left * 4  # Obnovenie rozlíšenia
...
log.basicConfig(filename='webcam.log', level=log.INFO) #fdfd
...
while True:
    ...
    yolo_results, face_locations, face_names = process_frame(frame, model, known_face_encodings, known_face_names)
    draw_frame(frame, yolo_results, face_locations, face_names)
